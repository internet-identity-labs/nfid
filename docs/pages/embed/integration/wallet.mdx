...
The `nfid.requestTransferNFT()` method requests approval to transfer an EXT NFT from the user to a designated address.
...
The `nfid.requestCanisterCall()` method requests approval to call another canister with the given `canisterId`, `method`, and `parameters` and return the response data.
